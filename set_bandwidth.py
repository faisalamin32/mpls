...
target = '192.168.5.10'
port = 6640
sock = 'ovs/var/run/openvswitch/db.sock'
...
def get_port_details(ovsdb):
    ...
    #switch_host = switch_host.get(host_mac)[-1]
...
def index():
    if request.method == 'POST':
        ovsdb = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ovsdb.connect((target,port))
        #ovsdb.connect(sock)
